...
Loaders (:mod:`.loaders`) stream rows purely from disk; this module owns the only side
effects — chunked bulk inserts and commits — keeping a pure-core/thin-shell split.
"""
...
class TabularSystem:
    ...
    def ingest_accounts(
        self, path: str, source_file: Optional[str] = None, on_batch: Optional[Callable[[int], None]] = None
    ) -> int:
        """Stream+bulk-insert ``HI-Large_accounts.csv``.

        Idempotent on ``(bank_id, account_number)``: re-ingesting the same file does not
        duplicate rows (``INSERT OR IGNORE``). Returns the number of rows newly inserted.
        ``on_batch``, if given, is called with the cumulative row count after each batch
        commits (e.g. for progress reporting).
        """
    ...
    def ingest_transactions(
        ...
        Returns the number of rows inserted. ``on_batch``, if given, is called with the
        cumulative row count after each batch commits.
        """
    ...
    def ingest(
        ...
        Lets the API route stay free of per-type branching. ``on_batch`` is threaded
        through to whichever of the three ingest methods is dispatched to.
        """
        dispatch = {
